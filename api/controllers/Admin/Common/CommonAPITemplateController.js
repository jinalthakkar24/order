...
module.exports = {
    ...
    async create(req, res) {
        ...
        try {
            await CommonValidator.validateCreateParams(options);
            ...
            return res.errorHandler(null, error);
    ...
    async update(req, res) {
        try {
            ...
            // required params check
            await CommonValidator.validateUpdateParams(options);

            // create update
            let paramsToUpdate = _.omit(params, 'id');
            let updatedRecord = await sails.models[modelName]
                .update({
                    id: params.id
                })
                .set(paramsToUpdate)
    ...
    async updatev1(req, res) {
        try {
            ...
            // required params check
            if (!await CommonValidatorService.validateRequiredUpdateParams(options)) {
                return res.badRequest(null, sails.config.message.BAD_REQUEST);
            }
            // create update
            let paramsToUpdate = _.omit(params, 'id');
            let updatedRecord = await `#{Model}`
                .update({
                    id: params.id
                })
                .set(paramsToUpdate)
