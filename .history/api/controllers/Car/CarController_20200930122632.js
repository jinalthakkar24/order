...
module.exports = {
    ...
    async bulkSequenceUpdate(req, res) {
        ...
        try {
            ...
            if (cars && cars.length) {
                for (let car of cars) {
                    const findSequence = _.find(params.sequences, { id: car.id });
                    if (findSequence) {
                        car.sortingSequence = findSequence.sequence;
                        let response = await Master.update({ id: car.id }).set(car).fetch();
                        updatedCars.push(_.first(response))
                    }
                }

                return res.ok(masterService.formatRecords({ cars: updatedCars }), sails.config.message.OK)
            }
            else {
